...
class JsHintCommand(sublime_plugin.WindowCommand):
	...
	def run(self):
		...
		jsscopes = view.find_by_selector('source.js')
		if(not jsscopes):
			return
		...
		def show_errors():
			...
			for scope in jsscopes:
				...
				if(self.jshint(source, hint_options)):
					self.panel.insert(edit, self.panel.size(), '0 errors, 0 warnings\n\n')
				else:
...
def on_syntax_error(view, context, source):
	try:
		context.eval(str(source))
	except Exception,ex:
		if('name' in dir(ex) and ex.name == 'SyntaxError'):
			err_region = sublime.Region(ex.startPos, ex.endPos+1)
...
class EventListener(sublime_plugin.EventListener):
	...
	def on_modified(self, view):
		...
		for scope in jsscopes:
			...
			sublime.set_timeout((lambda view,context,source : (lambda: on_syntax_error(view, context, source)))(view, self.ctx, source), 1)
